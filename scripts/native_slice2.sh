#!/usr/bin/env bash
# Stage 2 切片 2 native 验收(BOOTSTRAP_PLAN 附八):
#   frondc native --run <case>.frond → lower LLVM .obj(聚合:str/数组/
#   record 值 = ptr,bump arena @frond_heap 分配)→ 资产 lld 链接
...
# 语料:tests/fixtures/native_slice2/cases/*.frond(record/数组/str
# 三聚合 + 嵌套组合;不占 functional/ 目录——run_functional.sh 按目录
# 跑 frond run,本验收是脚本驱动)。
set -u
cd "$(dirname "$0")"
...
CASES="$ROOT/tests/fixtures/native_slice2/cases"
...
echo "native_slice2: $pass passed, $fail failed"
[ "$fail" -eq 0 ]
