#!/usr/bin/env bash
# Stage 2 切片 0 native 验收(BOOTSTRAP_PLAN 八):
...
# 语料:tests/fixtures/native_slice0/cases/*.frond(不占 functional/ 目录——run_functional.sh 按目录跑 frond run,本验收是脚本驱动)。
set -u
cd "$(dirname "$0")"
...
CASES="$ROOT/tests/fixtures/native_slice0/cases"
...
echo "native_slice0: $pass passed, $fail failed"
[ "$fail" -eq 0 ]
